...
void ofApp::setup(){
    ...
    quad.addVertex( glm::vec3(-0.6, -0.5, 0) );  // #0 : downer - left
    quad.addVertex( glm::vec3(-0.6,  0.5, 0) );  // #1 : upper  - left
    quad.addVertex( glm::vec3( 0.4,  0.5, 0) );  // #2 : upper  - right
    quad.addVertex( glm::vec3( 0.5,  0.5, 0) );  // #3 : upper  - right
    quad.addVertex( glm::vec3( 0.5, -0.5, 0) );  // #4 : downer - right
    quad.addVertex( glm::vec3(-0.5, -0.5, 0) );  // #5 : downer - left
    ...
    quad.addTexCoord( glm::vec2(1, 1) );     // #3 : upper  - right  |  color : (1,1,0,1) -> Yellow   (1,1)  =>  red:100% , green:100%  ( the blue:0%,  alpha:100% )  
    quad.addTexCoord( glm::vec2(1, 0) );     // #4 : downer - right  |  color : (1,0,0,1) -> Red      (1,0)  =>  red:100% , green:  0%  ( the blue:0%,  alpha:100% )  
    quad.addTexCoord( glm::vec2(0, 0) );     // #5 : downer - left   |  color : (0,0,0,1) -> Black    (0,0)  =>  red:  0% , green:  0%  ( the blue:0%,  alpha:100% )

    ofIndexType indices[IDX_COUNT] = 
    { 
        0, 1, 2,  // Upper-Left   triangle
        3, 4, 5   // Downer-Right triangle
    };
