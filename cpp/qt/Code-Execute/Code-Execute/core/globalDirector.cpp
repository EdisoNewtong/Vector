...
QString GlobalDirector::inneralLog0(ChInfo& chInfo)
{
    QString retstr;
    QTextStream ts(&retstr);
    if ( CmdOptions::needTraceParseTimeStep() ) {
        ts << chInfo.getPos().c_str() << " ";
    }
...
QString GlobalDirector::inneralLog1(ParserBase::E_PARSER_TYPE oldtp, ParserBase::E_PARSER_TYPE newtp)
{
    ...
    if ( CmdOptions::needTraceParseTimeStep()  ) {
        if ( oldtp == newtp ) {
            ts << EnumUtil::enumName(oldtp).c_str() << " ";
        } else {
            ts << EnumUtil::enumName(oldtp).c_str() << " -> " << EnumUtil::enumName(newtp).c_str() << " ";
        }
    }
...
QString GlobalDirector::inneralLog2(bool moveNext)
{
    QString retstr;
    QTextStream ts(&retstr);
