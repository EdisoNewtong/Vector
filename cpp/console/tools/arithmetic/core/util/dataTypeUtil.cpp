    ...
    for( auto i = static_cast<int>(E_TP_CHAR); i <= static_cast<int>(E_TP_DOUBLE); ++i )
    {
        for( auto j = static_cast<int>(E_TP_CHAR); j <= static_cast<int>(E_TP_DOUBLE); ++j )
        {
            ...
            if (         prpr.first.second == prpr.second.second 
                ...
                cout << "<" << EnumUtil::enumName(copyLeft) << "," << EnumUtil::enumName(copyRight) << ">   => "
                     << EnumUtil::enumName(prpr.first.second)  
                     << "  ( " << EnumUtil::enumName(copyLeft)  << " -> " << (copyLeft !=prpr.first.second  ? EnumUtil::enumName(prpr.first.second)  : "keep") << " ) "
                     << "  ( " << EnumUtil::enumName(copyRight) << " -> " << (copyRight!=prpr.second.second ? EnumUtil::enumName(prpr.second.second) : "keep") << " ) "
                     << endl;
