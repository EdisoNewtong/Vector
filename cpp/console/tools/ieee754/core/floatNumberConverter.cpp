...
/********************************************************************************
  ...
  IEEE 754 :   Float Point Number Binary Standard
...
----------------------------------------------------------------------------------------------------
  ...
  Float Number :   32 bits = 1 + 8 + 23
    ...
    1   Sign Bit      : 正负符号位 ， 0:正   1:负    ( special case (Zero) :   +0.0 -> 0  /   -0.0 -> 1 )
    8   Exponent Bits : Biased Exponet  //  2^(8-1) -1 = 127,    127 + n (n is the exponent of 2)
    23  Mantissa Bits : Mantissa (尾数) of float number data
...
e.g.
       ...
       =  1010101(B)    + 0.001(B)
       =  1010101.001(B)
       =  1.010101001 * (2^6)         // 二进制的科学计算法和十进制的略有不同， 十进制:  a * 10^n (  1 <= a < 10 的整数或小数 ) , 而二进制只有0/1 , 同时， ieee754 规定: a 的格式一定是 1.xxxxxxx * 2^n , 而不可能是 0.xxxxx * 2^n

85.125 > 0   ==>   1  Sign Bit      : 0
2^6          ==>   8  Exponet Bits  : ( 2^(8-1) -1 )  + 6 = 127 + 6 = 133   =>   10000101
1.010101001  ==>   23 Mantissa Bits : 010101001|00000000000000    将整数部分的前缀1. 删除后     小数部分为 010101001   =>   010101001 共9位，需要23位，因此向后补14位0 , 补齐23位

最终 85.125 的 ieee754 标准的 二进制数为
...
11   Exponent Bits : Biased Exponet  //  2^(11-1) -1 = 1023,    127 + n (n is the exponent of 2)
