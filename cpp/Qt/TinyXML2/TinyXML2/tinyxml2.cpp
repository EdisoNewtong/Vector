    ...
    *length = 0;
    ...
    if (*(p + 1) == '#' && *(p + 2)) {
        ...
        while (*q != terminator) {
            ...
            if (*q >= '0' && *q <= '9') {
                digit = static_cast<uint32_t>( *q - '0' );
            }
            else if (hex && (*q >= 'a' && *q <= 'f')) {
                digit = static_cast<uint32_t>( *q - 'a' + 10 );
            }
            else if (hex && (*q >= 'A' && *q <= 'F')) {
                digit = static_cast<uint32_t>( *q - 'A' + 10 );
            }
...
void XMLUtil::ToStr( uint64_t v, char* buffer, int bufferSize )
{
    ...
    TIXML_SNPRINTF(buffer, static_cast<size_t>(bufferSize), "%llu", static_cast<unsigned long long>(v));
}
...
void XMLPrinter::Write( const char* data, size_t size )
{
    if ( _fp ) {
        ...
        char* p = _buffer.PushArr( static_cast<size_t>(size) ) - 1;   // back up over the null terminator.
        memcpy( p, data, size );
        p[size] = 0;
    }
}
            ...
            if ( *q > 0 && *q < ENTITY_RANGE ) {
                ...
                if ( flag[static_cast<unsigned char>(*q)] ) {
                    while ( p < q ) {
                        ...
                        Write( p, static_cast<size_t>(toPrint) );
                        p += toPrint;
                    }
