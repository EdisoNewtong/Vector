...
void myhardcodewindow::innerBitsUpdate(int nBits)
{
    ...
    int leader2Idx = -1;
    for( int idx = 0; idx < m_byteBin.size(); ++idx ) {
        if ( m_byteBin[idx] != '0' ) {
            leader2Idx = idx;
            break;
        }
    ...
    for ( int idx2 = sc_binaryCtrlCnt-1; idx2>=0;  ) {
        if ( m_pBinAry[idx2] != nullptr && m_pBinIndexAry[idx2]!=nullptr ) {
            if ( dataIdx2 >=0 ) {
                ...
                auto bLeaderNoneZero = (dataIdx2 == leader2Idx);
    ...
    int leader8Idx = -1;
    for ( int idx = 0; idx < m_byteOct.size(); ++idx ) {
        if ( m_byteOct[idx] != '0' ) {
            leader8Idx = idx;
            break;
        }
    ...
    for ( int idx8 = sc_octCtrlCnt-1; idx8>=0;  ) {
        if ( m_pOctAry[idx8] != nullptr ) {
            if ( dataIdx8 >= 0 ) {
                ...
                auto bLeaderNoneZero = (dataIdx8 == leader8Idx);
    ...
    int leader16Idx = -1;
    for( int idx = 0; idx < m_byteHex.size(); ++idx ) {
        if ( m_byteHex[idx] != '0' ) {
            leader16Idx = idx;
            break;
        }
    ...
    for ( int idx16 = sc_hexCtrlCnt-1; idx16>=0;  ) {
        if ( m_pHexAry[idx16] != nullptr ) {
            if ( dataIdx16 >= 0 ) {
                ...
                auto bLeaderNoneZero = (dataIdx16 == leader16Idx);
