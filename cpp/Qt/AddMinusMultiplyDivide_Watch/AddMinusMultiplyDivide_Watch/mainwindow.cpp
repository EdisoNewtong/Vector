    ...
    , m_pSettingDlg(nullptr)
{
    ...
    m_tagStringMap.insert( static_cast<int>(E_ANSWER_TAG), "答案" );
...
void MainWindow::nextQuestion(bool correct)
{
...
void MainWindow::on_action_save_triggered()
{
    ...
    fileContent += ( QString("Correct Rate").leftJustified(12)  + VLINE  ); //  4: E_CORRECT_RATE_TAG
    fileContent += ( QString("Error Rate").leftJustified(12)  + VLINE  ); //  5: E_INCORRECT_RATE_TAG
    fileContent += ( QString("In-time Correct Rate").leftJustified(20)  + VLINE  ); //  6: E_CORRECT_IN_TIME_RATE_TAG 
    ...
    for( auto i = 0; i < m_questionIdx; ++i ) {
        ...
        for( auto j = static_cast<int>(E_QUESTION_TAG); j < E_TAG_MAX; ++j ) { 
            ...
            if ( item!=nullptr ) {
                QString content;
                // qDebug() << i << " , " << j << ". icon Name = " << item->icon().name();
                switch( j )
                {
                case E_QUESTION_TAG:
                    {
                ...
                case E_YES_NO_UNDECIDE_TAG:
                    {
                        ...
                        // if ( content.endsWith("undecided.png") ) {
                        //     content = "N/A";
...
void MainWindow::on_321goFinished()
{
    ...
    {
        QTableWidgetItem *newItem = new QTableWidgetItem( QIcon(QString(":/img/undecided.png")), "" );
...
void MainWindow::updateCurrentResultUI(bool correct)
{
    if ( m_questionIdx == 0 ) {
        return;
    }
    ...
    // yes/no/ haven't decided yet
    {
        ...
        if ( item != nullptr ) {
            ...
            item->setIcon( QIcon(correct ? QString(":/img/yes.png") : QString(":/img/no.png")) );
    ...
    // answer
    {
        ...
        if ( item != nullptr ) {
            if ( questionInfo.contains("÷") && m_currentModAnswer.toInt()!=0 ) {
                QString modAnswer = QString("%1  ... %2").arg(m_currentAnswer).arg( m_currentModAnswer ); 
