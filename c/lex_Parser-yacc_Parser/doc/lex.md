...
2. **Generate the Lexer**: Run `lex` on the input file to generate the C code for the lexer.
    ```bash
    lex example.l
    ```
...
3. **Compile the Lexer**: Compile the generated C code using a C compiler like `gcc`.
    ...
    ```
    The `-ll` option links against the `libl` library, which is necessary for the lexer.
