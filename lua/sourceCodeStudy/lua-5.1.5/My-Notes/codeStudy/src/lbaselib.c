...
static void getfunc (lua_State *L, int opt) {
  ...
  if ( lua_type(L, 1) == LUA_TFUNCTION ) {
    ...
    lua_Debug ar;
    /*          opt ? luaL_optint(L, 1, 1) : luaL_checkint(L, 1); */
...
static int luaB_select (lua_State *L) {
  int n = lua_gettop(L);
  if (lua_type(L, 1) == LUA_TSTRING && *lua_tolstring(L, 1, NULL) == '#') {
...
static int luaB_newproxy (lua_State *L) {
  ...
  /*        lua_type(L, (1)) == 1      */
    ...
    lua_pushboolean(L, 1);
    /*            lua_upvalueindex(1)    */
    lua_rawset(L, (LUA_GLOBALSINDEX-1)); /* weaktable[m] = true */
  }
  else {
    int validproxy = 0; /* to check if weaktable[metatable(u)] == true */
    if (lua_getmetatable(L, 1)) {
      /*            lua_upvalueindex(1)    */
      lua_rawget(L, (LUA_GLOBALSINDEX-1));
...
static int luaB_auxwrap (lua_State *L) {
  /*                              lua_upvalueindex(1) */
  lua_State *co = lua_tothread(L, (LUA_GLOBALSINDEX-1));
...
static int luaB_cocreate (lua_State *L) {
  ...
  /*            lua_type(L, (1)) == 6  */
  ( (void)( ( ( lua_isfunction(L, 1) ) && !lua_iscfunction(L, 1) ) || luaL_argerror(L, 1, "Lua function expected") ) );
