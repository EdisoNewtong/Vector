...
static int os_date (lua_State *L) {
  /*              luaL_optstring(L, 1, "%c"); */
  const char *s = luaL_optlstring(L, 1, "%c", NULL);
  /*         luaL_opt(L, (time_t)luaL_checknumber, 2, time(NULL)); */
  time_t t =   (lua_type(L, ((2))) <= 0) 
             ? (time(NULL)) 
  ...
  if (stm == NULL) { /* invalid date? */
    ...
    for (; *s; s++) {
      if (*s != '%' || *(s + 1) == '\0') { /* no conversion specifier? */
        /* luaL_addchar(&b, *s); */
        (
         (void)((&b)->p < ((&b)->buffer+8192) || luaL_prepbuffer(&b)), 
         (*(&b)->p++ = (char)(*s))
        );
      } else {
        size_t reslen;
